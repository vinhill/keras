...
def get_dataset(input_context=None, batch_size=None):
  ...
  dataset = dataset.batch(batch_size).repeat(100)
  return dataset
...
class KerasPremadeModelsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def skip_unsupported_combination_test(self, distribution,
                                        use_dataset_creator):
  ...
  def test_linear_model(self, distribution, use_dataset_creator, data_fn):
    self.skip_unsupported_combination_test(distribution, use_dataset_creator)
